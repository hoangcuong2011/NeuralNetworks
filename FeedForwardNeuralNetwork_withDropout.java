 ...
 */
public class FeedForwardNeuralNetwork_withDropout {
    ...
    public void backpropagationNLayers(double xs_training[][], double ys_training[][], 
        ...
        for (int iteration = 0; iteration < 30000; iteration++) {
            ...
            for (int k = 0; k < total_layers; k++) {
                dropoutMatrix[k] = new boolean[hidden_units[k]];
            }
            for (int k = 1; k < total_layers-1; k++) {
                dropoutMatrix[k] = new boolean[hidden_units[k]];
                for (int i = 0; i < hidden_units[k]; i++) {
                    boolean b = getRandomBoolean((1.0f - (float) dropout_rate));
                    dropoutMatrix[k][i] = b;
                }
            }
            for (int k = 1; k < total_layers; k++) {
            ...
            if ((iteration + 1) % 1000 == 0) {
                ...
                double hs[][][] = computeForward(total_layers, minibatch, hidden_units, xs_shuffle, wxhs, 
                        biases, false, wxhs_dropout, dropout_rate);
            ...
            double hs[][][] = 
                    computeForward(total_layers, minibatch, hidden_units, xs_shuffle, wxhs, biases,
                            true, wxhs_dropout, dropout_rate);
        ...
        D3: number of neurons of the layer
            ...
            CalculateSignalErrorsCrossEntropy(hs, ys_shuffle, wxhs, 
                    ...
                    total_layers, signalErrors, wxhs_dropout, dropout_rate);

            updateWeights(hs, wxhs, biases, hidden_units, minibatch, total_layers, signalErrors,
                    wxhs_update, biases_update, constant, wxhs_dropout, dropout_rate, cache, gamma);
    ...
    public static void main(String argss[]) throws IOException {
        FeedForwardNeuralNetwork_withDropout program = new FeedForwardNeuralNetwork_withDropout();
